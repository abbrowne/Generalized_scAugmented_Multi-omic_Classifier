...
for(temp_input_i in 1:length(input_expression_set)){
  ...
  input_gsva_set[[temp_name]] <- gsva(gsvaParam(input_expression_set[[temp_name]],hp_gmt))
}
temp_gsva <- t(input_gsva_set$log2CPM)[rownames(input_meta),]
...
fit <- rpart(temp_formula, data = temp_meta, method = "exp")
