...
fit <- rpart(temp_formula, data = temp_meta, method = "exp", control = rpart.control(cp=0.02,maxdepth=4,
                                                                                     minsplit = round(nrow(input_meta)/5),
                                                                                     minbucket = round(nrow(input_meta)/10)))
...
my_gene_set <- all_gmts$CIBERSORT.Neutrophils

gene_set_list <- list(my_gene_set)

# Add module score to Seurat object
seurat_obj <- AddModuleScore(
    object   = input_scData,
    features = gene_set_list,
    name     = "MyGeneSet"
)
...
DotPlot(
    seurat_obj, 
    features  = my_gene_set, 
    group.by  = "Cell_type"
) + RotatedAxis()

RidgePlot(seurat_obj, features = "MyGeneSet1", group.by = "Cell_type")
...
test_markers <- FindMarkers(
    ...
    logfc.threshold = 0.25,          # filter out genes with low logFC
...
##Use subset of genes from scRNAseq to identify classifier
