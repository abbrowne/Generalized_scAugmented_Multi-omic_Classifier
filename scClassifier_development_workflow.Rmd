...
find_cellEnrichedPrognostic_scores <- function(input_meta,
                                               ...
                                               initial_minsplit_fraction=0.2,
                                               initial_minbucket_fraction=0.1,
                                               minsplit_multiple=0.5,
                                               minbucket_multiple=0.5,
  ...
  while(flag_primary_cutpoint_sc_enrichment == FALSE & total_iterations <= max_iterations){
    if(total_iterations > 0){
      temp_minsplit <- round(temp_minsplit * minsplit_multiple)
      temp_minbucket <- round(temp_minbucket * minbucket_multiple)
      temp_formula <- as.formula(paste0("Surv(",input_outcome_time,", ",input_outcome_result,") ~ ",
    ...
    for(temp_cp_i in 1:length(temp_cp_set)){
      ...
      for(temp_prog_feature_i in 1:max_prog_features_per_fit){
        ...
        if(temp_coxph_pval < 0.05){
          ...
          if(sum(comp_result$max_module_score >= comp_result$other_module_scores*2) == length(comp_result$other_module_scores)){
            ...
            #                        data = temp_meta)
            temp_survFit <- survfit(as.formula(paste0("Surv(", 
                                                      input_outcome_time, ", ", input_outcome_result, 
                                                      ") ~ test_surv_group")),data = temp_meta)
...
test_result <- find_cellEnrichedPrognostic_scores(test_meta,test_outcome_time,test_outcome_result,test_sc_dataset,test_gmts)
